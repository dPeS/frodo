...
class InitDatabaseMixin:
    ...
    def _sql_setup_init_database(self):
        ...
        # ORDER TYPE
        cursor.execute(
            "create table {prefix}_ea_type("
            "code int,"
            "container_id int references {prefix}_ea_container(id),"
            "max_quantity int,"
            "primary key (code, container_id) "
            ")".format(prefix=PREFIX)
        )
        # ORDER
        cursor.execute(
            "create table {prefix}_ea_order("
            "id serial primary key,"
            "order_duration tstzrange,"
            "order_timeout timestamp,"
            "container_id int,"
            "type_code int,"
        ...
        cursor.execute(
            ...
            "returns bigint as "
            "$$ "
            "select (select max_quantity from {prefix}_ea_type "
            "where container_id = $2 and code = $3)"
            " - "
        ...
        cursor.execute(
            "alter table {prefix}_ea_order add CHECK "
            "({prefix}_ea_check_orders_fn("
            "order_duration,"
            "container_id,"
            "type_code) > 0)"
            "".format(prefix=PREFIX)

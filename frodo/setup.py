...
class InitDatabaseMixin:
    ...
    def _sql_setup_init_database(self):
        ...
        # CONTAINER
        cursor.execute(("create table {}_ea_container(id serial primary key)").format(PREFIX))
        # ORDER TYPE
        cursor.execute(("create table {prefix}_ea_type(code int, "
                        "container_id int references {prefix}_ea_container(id), "
                        "max_quantity int, "
                        "primary key(code, container_id) )").format(prefix=PREFIX))
        # ORDER
        cursor.execute(("create table {prefix}_ea_order(id serial primary key, "
                        "order_duration tstzrange, "
                        "container_id int, "
                        "type_code int, "
                        "foreign key(container_id, type_code) references {prefix}_ea_type(container_id, code))").format(prefix=PREFIX))

        ### TOOLS
        # internal FUNCTION used in constraint
        cursor.execute(("create or replace function {prefix}_ea_check_orders_fn(tstzrange, int, int) "
                        "returns bigint as "
                        "$$ "
                        "select (select max_quantity from {prefix}_ea_type where container_id = $2 and code = $3) "
                        " - count(1) from {prefix}_ea_order where not isempty(order_duration * $1) "
